...
async def mpx(ctx, *args, **kwargs):
    ...
    try:
        ...
        symbols = argx.symbols
        if len(argx.symbols) == 1 and ',' in argx.symbols[0]:
            symbols = split(argx.symbols)  # argx[0].replace(' ', '').split(',')
        ...
        if PYTHON_ENVIRONMENT == 'production':
            ...
            symbols, mean, corr = calc_correlation(symbols, argx.days)
            result = optimize_profit(symbols, mean, corr, bank_interest_rate=argx.interest_rate)
